...
module.exports = {
  ...
  apply(ctx) {
    ...
    async function removeFile(path) {
      ...
      const spec = win
        ? {
            // 文件不存在也返回成功（live-only 会话可能没有持久化文件）
            argv: [
              'powershell',
              '-NoProfile',
              '-NonInteractive',
              '-Command',
              "Remove-Item -LiteralPath '" + path.replace(/'/g, "''") + "' -Force -ErrorAction SilentlyContinue; exit 0",
            ],
          }
        : { argv: ['rm', '-f', '--', path] }
    ...
    async function deleteOne(id) {
      ...
      if (live) {
        ...
        // 让 live 会话离开内存 store，触发官方 session/disposed -> host/session-removed，
        // 客户端会立即移除该会话（否则残留列表并掉入"未分组"）
        try {
          const entry = store.store?.get(id)
          if (entry && typeof entry.detach === 'function') entry.detach()
        } catch (e) {
          log('store detach failed', id, e)
        }
      }
